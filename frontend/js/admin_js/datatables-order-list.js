window.addEventListener("DOMContentLoaded", (event) => {
  ...
  function getCompletionWeight(isComplete) {
    switch (isComplete) {
      case "N":
        return 0; // 'No' comes first
      case "Y":
        return 1; // 'Yes' comes second
      default:
        return 2; // 'N/A' or others come last
    }
  }
  ...
  function prefillChangeModal(order) {
    ...
    document.getElementById("currentOrderItem").value =
      order.item_name || "N/A";
  ...
  function prefillRemoveModal(order) {
    ...
    document.getElementById("removeOrderItem").textContent =
      order.item_name || "this item";
  }
  ...
  function setupModalListeners(tableElement) {
    ...
    document
      .getElementById("saveStatusChange")
      .addEventListener("click", async function () {
        ...
        const modalElement = document.getElementById("changeStatusModal");
        const originalQuantity = modalElement.getAttribute(
          "data-original-quantity"
        );
        const originalStatus = modalElement.getAttribute(
        ...
        try {
          ...
          if (response.ok) {
            ...
            const changeModalElement =
              document.getElementById("changeStatusModal");
            const modalInstance =
              bootstrap.Modal.getInstance(changeModalElement);
            if (modalInstance) modalInstance.hide();
    ...
    document
      .getElementById("confirmRemoveOrder")
      .addEventListener("click", async function () {
        ...
        try {
          ...
          if (response.message !== undefined) {
            ...
            const removeModalElement =
              document.getElementById("removeOrderModal");
            const modalInstance =
              bootstrap.Modal.getInstance(removeModalElement);
            if (modalInstance) modalInstance.hide();
            await initializeDataTable();
            ...
            alert(
              `Failed to remove order. Server response status: ${responseRaw.status}`
            );
          }
        } catch (error) {
    ...
    if (!token) {
      ...
      if (datatablesOrders)
        datatablesOrders.innerHTML =
          '<tr><td colspan="9" class="text-center">Authentication Required.</td></tr>';
      return;
    }
    ...
    try {
      ...
      if (!responseRaw.ok) {
        ...
        if (responseRaw.status === 401) {
          throw new Error(
            `Authorization Failed. Status: ${responseRaw.status}`
          );
        }
      ...
      if (!responseData.success || !Array.isArray(responseData.data)) {
        console.error(
          "API response format error or unsuccessful:",
          responseData
        );
      ...
      if (datatablesOrders)
        datatablesOrders.innerHTML =
          '<tr><td colspan="9" class="text-center">Error loading data. Check console.</td></tr>';
      return;
    }
    ...
    if (datatablesOrders) {
      ...
      if (simpleDataTableInstance) {
        try {
          ...
          console.error(
            "Failed to destroy existing table instance. Forcing new initialization.",
            e
          );
        }
      }
      ...
      simpleDataTableInstance = new simpleDatatables.DataTable(
        datatablesOrders,
        {
          data: {
            headings: headings,
