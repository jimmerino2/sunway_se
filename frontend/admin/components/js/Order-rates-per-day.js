...
document.addEventListener('DOMContentLoaded', () => {
  ...
  checkTokenIntervalId = setInterval(() => {
    ...
    if (token) {
      ...
      initializeChartData(); // Logic update: api.js retrieves the token internally
    }
...
async function initializeChartData() {
  ...
  try {
    // --- Initial Fetch Call (Refactored to use api.js) ---
    const apiResponse = await getApiResponse(apiUrl);
    ...
    if (apiResponse && apiResponse.success && Array.isArray(apiResponse.data)) {
      ...
      console.error('API response was not successful or data is missing:', apiResponse ? apiResponse.message : 'Unknown Error');
    }
  } catch (error) {
...
async function fetchAndRefreshChart() {
  ...
  // Note: api.js handles token retrieval internally
  ...
  try {
    ...
    if (apiResponse && apiResponse.success && Array.isArray(apiResponse.data)) {
      ...
      console.error('Auto-refresh: API response error:', apiResponse ? apiResponse.message : 'Unknown Error');
    }
  } catch (error) {
