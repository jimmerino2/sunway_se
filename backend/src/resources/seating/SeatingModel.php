...
class SeatingModel {
    private PDO $db;
    private string $tableName = 'seating'; 
    ...
    public function checkUniqueTableNo($id, $table_no) {
        $sql = "SELECT * FROM {$this->tableName} WHERE id != ? AND table_no = ? LIMIT 1";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$id, $table_no]);
        return $stmt->fetch(PDO::FETCH_ASSOC); 
    }

    public function saveSeating($data) {
        // Remove false parameters
        $validColumns = array_column($this->columns, 'name');
    ...
    public function updateSeating(array $data): bool {
    ...
    public function deleteSeating(int $id): bool {
